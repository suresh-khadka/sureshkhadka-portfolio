...
class NotebookListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        # Deriving title from filename or request data
        title = self.request.data.get('title')
        if not title:
            title = file_obj.name.replace('.ipynb', '').replace('_', ' ').title()

        # We use serializer.save() but since section and storage_path are read_only,
        # and title is handled here, we pass them as keyword arguments.
        # Note: notebook_id is generated by the model's UUIDField.

        # To ensure the notebook record is created before upload (to get the ID),
        # we save it first.
...
class NotebookDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        # Check if a new file is being uploaded for replacement
        ...
        if file_obj:
            try:
                ...
                # Delete old file first to avoid duplication if path changes (though it shouldn't here)
