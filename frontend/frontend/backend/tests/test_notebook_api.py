...
def test_section_reorder():
    client = APIClient()
    client.force_authenticate(user=None) # Assuming admin check is handle by IsAdminOrReadOnly
    # Need a superuser for the test
    ...
    response = client.patch(url, {'order': [s2.id, s1.id]})
